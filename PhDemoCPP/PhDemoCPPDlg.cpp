...
void CPhDemoCPPDlg::SelectCineFromCombo()
{
	...
	if (pSelCam != NULL)
	{
        int firstFlashCine = pSelCam->GetFirstFlashCine();	//camera->GetFirstFlashCine()
...
void CPhDemoCPPDlg::SetAcquisitionParams()
{
	...
	if (pSelCam!=NULL && !pSelCam->IsCinePartitionStored(pSelCam->GetSelectedCinePartNo()))
	{
		...
		ACQUIPARAMS acqParams;		// _JF 定义采集参数变量
		pSelCam->GetAcquisitionParameters(pSelCam->GetSelectedCinePartNo(), &acqParams);	//_JF 读取采集参数
		//set resolution
		CString resStr;
		POINT resolution;
		...
		pSelCam->SetAcquisitionParameters(pSelCam->GetSelectedCinePartNo(), &acqParams);	//_JF 设置采集参数
